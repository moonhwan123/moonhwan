...
<script>
	function send(){
		...
		for(x=0; x<form_name.name.value.length; x++){
			if(form_name.name.value.charAt(x)==" "){
				alert("공백no");
				return;
			}
		}
		...
		for(var x = 0 , flag = 0; x < form_name.gubun.length; x++){
			if(form_name.gubun[x].checked){
				flag=1;
				break;
			}
		}
		...
		for(var x = 0,flag = 0; x < form_name.fa.length; x++){
			if(form_name.fa[x].checked){
				flag=1;
				break;
			}
		}
		...
		form_name.action="..\\input_pro.jsp";
		form_name.submit();
		alert("회원등록에 성공 하였습니다.");
	}
</script>
</head>
